...
class Yolo5s:
    ...
    def predict(self, img_path, conf_thres=0.25, iou_thres=0.45, agnostic_nms=False, max_det=1000):
        img = self._preproc(img_path)
        pred = self.sess.run([self.output_name], {self.input_name: img})
        pred = torch.from_numpy(np.asarray(pred)[0])
        pred = self._non_max_suppression(pred, conf_thres, iou_thres, self.classes, agnostic_nms, max_det=max_det)

        return pred

    # preproc img source to img
    def _preproc(self, img_path):
...
if __name__ == "__main__":
    yolo5s = Yolo5s("./models/yolov5s.onnx")
